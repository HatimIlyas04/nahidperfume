...
.pd-hero { display: grid; grid-template-columns: 1fr 1fr; gap: 48px; }
@media (max-width: 900px) { .pd-hero { grid-template-columns: 1fr; } }
.pd-media { border-radius: var(--radius-xl); overflow: hidden; aspect-ratio: 4/5; background: var(--gray-100); box-shadow: var(--shadow-lg); }
.pd-media img { width: 100%; height: 100%; object-fit: cover; }
.pd-info { display: flex; flex-direction: column; }
.pd-title { font-family: var(--font-display); font-size: clamp(2rem, 3.6vw, 2.8rem); font-weight: 500; margin-bottom: 10px; }
.pd-desc { color: var(--text-light); line-height: 1.7; margin-bottom: 20px; }
.pd-price-row { display: flex; align-items: baseline; gap: 10px; margin-bottom: 24px; }
.pd-price { font-family: var(--font-display); font-size: 2.2rem; font-weight: 600; }
.pd-compare { font-size: 1rem; color: var(--text-muted); text-decoration: line-through; }
.pd-actions { display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 8px; }
.pd-icon-btn {
...
.pd-perfumes-title { font-family: var(--font-display); font-size: 1.6rem; font-weight: 500; }
.pd-customize-toggle {
...
.pd-perfumes-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 18px; }
@media (max-width: 760px) { .pd-perfumes-grid { grid-template-columns: repeat(2, 1fr); } }
.pd-perfume-card { border-radius: var(--radius-lg); border: 1px solid var(--border-light); overflow: hidden; background: white; transition: var(--transition); position: relative; }
.pd-perfume-card.changed { border-color: var(--primary); box-shadow: 0 0 0 3px rgba(239,119,106,0.12); }
.pd-perfume-img-btn { display: block; width: 100%; aspect-ratio: 1; background: var(--gray-100); border: none; padding: 0; cursor: pointer; }
.pd-perfume-img-btn img { width: 100%; height: 100%; object-fit: cover; transition: transform 0.4s; }
.pd-perfume-img-btn:hover img { transform: scale(1.06); }
.pd-perfume-body { padding: 12px 14px; }
.pd-perfume-name { font-size: 0.88rem; font-weight: 600; color: var(--secondary); }
...
.pd-changed-tag { position: absolute; top: 10px; left: 10px; background: var(--primary); color: white; font-size: 0.6rem; font-weight: 700; padding: 3px 9px; border-radius: var(--radius-full); text-transform: uppercase; }
.pd-sticky-cta { position: sticky; bottom: 0; background: white; border-top: 1px solid var(--border-light); padding: 16px 32px; margin-top: 40px; display: flex; justify-content: flex-end; gap: 12px; box-shadow: 0 -6px 24px rgba(0,0,0,0.05); }
`;
  ...
  return (
    <>
      ...
      <div className="pd-wrap">
          ...
          <div className="pd-info">
            ...
            <div className="pd-price-row">
              ...
              {pack.compare_at_price && Number(pack.compare_at_price) > Number(pack.price) && (
                <span className="pd-compare">{Math.round(pack.compare_at_price)} MAD</span>
              )}
            </div>
        ...
        <div className="pd-perfumes-section">
      ...
      {hasChanges && (
        <div className="pd-sticky-cta">
          <span style={{ alignSelf: "center", fontSize: "0.85rem", color: "var(--text-light)", marginRight: "auto" }}>
